<template>
  <Layout>
    <div class="flex items-center justify-between py-8">
      <h1
        class="text-3xl font-bold text-gray-800 capitalize lg:ml-4 lg:text-4xl"
      >
        {{ $page.tag.title }}
      </h1>
      <p class="hidden font-semibold text-blue-700 md:block">
        {{ $page.tag.belongsTo.totalCount }} Articles
      </p>
    </div>
      ...
      <div class="max-w-xl px-6 py-6 bg-white rounded-lg shadow">
        <article>
          <g-link class="relative block" :to="post.node.path">
            <h3
              class="mb-2 text-xl font-bold leading-tight transition duration-75 ease-in-out hover:text-blue-800"
            >
              {{ post.node.title }}
            </h3>
            ...
            <div class="mb-8">
              <g-link
                :to="post.node.path"
                class="font-medium transition duration-75 ease-in-out text hover:text-blue-800"
                >Read More</g-link
              >
            </div>
          </g-link>
